...
let config = {
  ...
  BLOOM_INTENSITY: 0.14, // 0.8,
  BLOOM_THRESHOLD: 0.74, // 0.6,
  BLOOM_SOFT_KNEE: 0.7,
  SUNRAYS: false, // true,
...
function multipleSplats(amount) {
  for (let i = 0; i < amount; i++) {
    ...
    const color = generateColor(left);
    color.r *= 10.0;
    color.g *= 10.0;
    color.b *= 10.0;
...
function updatePointerMoveData(pointer, posX, posY) {
  let left;
  if (posX > canvas.width / 2) left = false;
  else left = true;
  pointer.color = generateColor(left);
  pointer.prevTexcoordX = pointer.texcoordX;
  pointer.prevTexcoordY = pointer.texcoordY;
  pointer.texcoordX = posX / canvas.width;
  pointer.texcoordY = 1.0 - posY / canvas.height;
...
function generateColor(left) {
  let c = HSVtoRGB(Math.random(), 1.0, 1.0); // 0.1 0.1 0.1 white (for white black, color invert css)
  //console.log(c);
  if (left) {
    c.r = 0.129 * 0.5; // 0.12275572707416291;
    c.g = 0.098 * 0.5; // 0;
    c.b = 0.194 * 0.5;
  } else {
    c.r = 0.049 * 0.5; // 0.12275572707416291;
    c.g = 0.187 * 0.5; // 0;
    c.b = 0.148 * 0.5;
  }
  //----------------
  // logo purple 0.129 0.098 0.194
  // logo green 0.049 0.187 0.148
  // * 0.5 for less light
